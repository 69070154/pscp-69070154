...
```text
input: เดือน เป็นจำนวนเต็ม และ วัน เป็นจำนวนเต็ม
output: ชื่อ weather (spring,fall,wintr,summer
constraints:ควรพิจารณาถ้าใส่เลข 0 หรือ เลขติดลบ
```
...
```text
step 1:รับค่า month and day
step 2:ตั้งเงื่อนไข if elif ถ้า month == 1 or month == 2 or month == 3 อยู่ใน monthให้ พิม winter
step 3: ตั้งเงื่อนไข if ต่อจาก step 2 อีกที ว่า ถ้า month == 3 and day >= 21 print spring
step 4:แล้วก็ทำให้เหมือน step 2,3 เลย แต่เปลี่ยน month == เป็นเลขอื่นๆ กับ weather ที่ print ออกมาตามเดือนที่กำหนด
step 5: ถ้าไม่ตรงเงื่อนไข อะไรหรือเลข ไม่ตรงกับเงื่อนไข else  จะพิมพ์ error 
```
...
```text
วิธีสุดท้ายที่ใช้เหมือนกับแผนแรกเลย แต่เปลี่ยน รูป code ให้ดูง่ายขึ้น เพราะ ผมใช้ month == or month == month == ทำให้ code มันยาวแล้วดูไม่สวย ทำให้ต้องมาใช้ in แทน โดนเขียนว่า month in (1,2,3) ทำให้ดูดีขึ้นกว่าเดิม
...
```text
ผมอยากให้มันขึ้น error เพราะเนื่องจากค่าที่ input มาไม่ตรงเงื่อนไข เลยอยากให้ขึ้น error มา ตาม code ที่ผมเขียนไว้เลย
```
...
```text
เพราะ month ==3 and day >= 21 print spring ทำให้ ตัวของ day เราจะใส่มากกว่า เท่าไรก็ได้  สุดท้าย มันจะ print spring อยู่ดี
```
...
```text
เพราะ month ==3 and day >= 21 print spring แต่ month อันนี้ == 2 เลยไม่ได้ทำให้ มัน print spring ออกมา แม้ วันที่ จะตรงเงื่อนไข เป็นเพราะ and ต้องการ T ทั้งคู่
```
