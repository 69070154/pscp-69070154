...
| OJ submission ID, if submitted | 599568 |
| OJ status | Pass |
...
```text
ฉันถาม AI ให้ช่วย ว่าแผนแรกที่ฉันว่างไว้มันถูกต้องไหมเพราะไม่รู้ตอนแรกว่าถ้า ค่า input เป็น 100000 จะทำยังไง ทำให้ต้องมาคิดว่าจะต้องทำไงถึงจะ output ออกมาถูกต้องเลยถามอีกว่าจะต้องใช้การคำนวณยังไงเพื่อหา output ที่ถูกต้อง
```
...
```text
ช่วยให้สังเกตุว่า input 1<= x <= 100000 เพราะตอนแรกทำแค่เลขจำกัดทำให้ ต้องเริ่มใหม่ตั้งแต่แรก
```
...
```text
ฉัน ตรวจสอบค่า ที่ input หลังจาก ปรับแก้ตามที่ AI สอนว่าจะต้องใช้อะไรถึงจะทำให้ Output ออกมาถูกต้อง
```
...
```text
ฉันได้เรียนรู้ว่า เราควรจะตรวจสอบ input ให้ดีว่าเขากำหนดมาว่าที่จำนวนที่เท่าไรถึงเท่าไร และการการใช้สูตรหา ว่าคำนวณ ยังไงเพื่อหาว่าเราจะแพงกำแพงน้อยที่สุดยังไง
```
