...
ค่า x เป็นจำนวนเต็ม
Output:
จำนวนกำแพงน้อยที่สุดที่ต้ิงพังไป ถึงหมายเลข 1 โดยเริ่มจากห้อง x ถ้าไม่ต้องพักกำแพง ให้ตอบ 0
Constraints:
input เป็นจำนวนควรพิจารณา ถ้า input เป็น 0 และ ติดลบ เพราะ modulo ยังใช้ได้
```
...
```text
Step1: รับค่า x เป็นจำนวนเต็ม
Step2: ถ้าอยู่ ตัวเลขใน ก็ print จำนวนที่ต้องใช้ในการพังกำแพง
Step3: ถ้าเป็น 1 ก็ print 0
```
...
```text
วิธีสุดท้ายที่ใช้จริง คือการ import math  จากนั้นรับค่า x มาแล้ว แล้วเอา x มา sqrt แล้ว ceil เพื่อให้ตัวเลขไม่มีเศษเพื่อให้รู้ว่า x ที่input อยู่ชั้นอะไร แล้วใว้ในค่า l จากนั้นให้ num รับค่า x - (l -1) ** 2 เพื่อหาว่า ตัวนั้นเป็นเลขคู่หรือคี่
จากนั้น ถ้า num % 2 ได้ 0 เพื่อให้รู้ว่าเป็นเลขคี่ เพื่อ print(2 * l - 2) แต่ถ้าไม่ตรงเงื่อนไข ก็จะเป็นเลขคู่ แล้ว print(2 * l -1)
```
...
```text
เพราะตัวเลขเยอะทำให้ต้องใช้การพังกำแพง ที่เยอะ ทำให้การคำนวณอาจจะผิดพลาดเลยต้องลองทดสอบว่าถูกต้องไหม
```
...
```text
เพราะเนื่องจากเราใส่ค่าเลขติดลบไป ทำให้อยากรู้ว่าจะเกิดไรขึ้น ทำให้รู้ว่าตัวของ math.sqrt ไม่สามารถใส่ค่าติดลบในนี้ได้
```
...
```text
เพื่อทดสอบ ในอีก output หนึ่งถ้า x เป็น  1 ก็ต้อง print("0")ออกมาเพราะไม่ต้องกำแพงเพราะอยู่ที่จุดหมายแล้ว
```
